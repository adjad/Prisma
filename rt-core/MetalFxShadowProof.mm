...
int main(){@autoreleasepool{
 ...
 Mesh floor;floor.plane(2,0xffffffff);check(floor.upload(ctx),"floor");
 ...
 p.dimensions[3]=32768u|1u|16u|64u|128u;p.controls[2]=0;p.controls[3]=1;p.sun[3]=0;
 auto actor=[&](float x){
  RtVertex vertices[]={{x,6,5},{x+2,6,5},{x+2,8,5},{x,6,5},{x+2,8,5},{x,8,5}};
  RtMaterial m={};for(float &v:m.tint)v=1;m.surface[0]=1;m.optics[0]=1.5;m.optics[2]=1;m.image[1]=m.image[2]=1;RtMaterial materials[]={m,m};uint32_t white=0xffffffff;
 ...
 double initialOld=region(2,4.75),initialNew=region(6,4.75);
 check(initialOld<.05&&initialNew>.75,"offscreen actor casts correct geometric shadow");
 check(actor(5),"move actor");values=frame(.9);double movedOld=region(2,4.75),movedNew=region(6,4.75);
 check(movedOld>.75&&movedNew<.05,"first moved frame clears old shadow and shades new position");
 for(unsigned f=0;f<40;f++){values=frame(.9);check(region(2,4.75)>.75&&region(6,4.75)<.05,"no trail while moving shadow settles");}
 check(rt_set_dynamic_mesh(ctx,nullptr,0,nullptr,nullptr,0),"remove actor");values=frame(.9);double removedOld=region(2,4.75),removedNew=region(6,4.75);
 check(removedOld>.75&&removedNew>.75,"first removal frame clears stale shadow");
 for(unsigned f=0;f<40;f++){values=frame(.9);check(region(2,4.75)>.75&&region(6,4.75)>.75,"no shadow trail after removal interval expires");}
 ...
 printf("{\"passed\":%s,\"failures\":%d,\"initialShadow\":%.8f,\"initialLit\":%.8f,\"movedOldLocation\":%.8f,\"movedNewShadow\":%.8f,\"removedOldLocation\":%.8f,\"removedNewLocation\":%.8f,\"completedFrames\":%llu,\"historyResets\":%llu}\n",failures?"false":"true",failures,initialOld,initialNew,movedOld,movedNew,removedOld,removedNew,(unsigned long long)fx.completed_frames,(unsigned long long)fx.history_resets);
