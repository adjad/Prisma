...
int main(){@autoreleasepool{
    ...
    for(unsigned mode=0;mode<2;mode++){
        std::vector<double> sum(size*size),squared(size*size);p.dimensions[3]=8|64|16|(mode?128:0);p.dimensions[2]=0;
        for(unsigned frame=0;frame<frames;frame++){@autoreleasepool{
            ...
            // Unrelated geometry behind the camera changes every frame. It is
            // farther than the AO range and outside the receiver's sun rays.
            std::vector<RtVertex> actor;float shift=.001f*frame;
            quad(actor,{.1f+shift,.1f,6},{.2f+shift,.1f,6},{.2f+shift,.2f,6},{.1f+shift,.2f,6});
            RtMaterial material={};for(float &v:material.tint)v=1;material.surface[0]=1;material.optics[0]=1.5;material.optics[2]=1;material.image[1]=material.image[2]=1;
            RtMaterial materials[2]={material,material};uint32_t white=0xffffffff;
            if(!rt_set_dynamic_mesh(ctx,actor.data(),unsigned(actor.size()),materials,&white,1))return 1;
    ...
    bool passed=rejectsInvalid&&before.supported&&before.active&&before.completed_frames==2*frames&&before.history_resets==frames+1&&after.history_resets==frames+2&&!alphaErrors&&!bypassErrors&&variance[0]>1e-6&&variance[1]<variance[0]*.7&&std::abs(mean[1]-mean[0])<.12&&resetMean>.98;
    printf("{\"passed\":%s,\"denoiserExecuted\":true,\"encodedFrames\":%llu,\"completedFrames\":%llu,\"historyResets\":%llu,\"globalResetVariance\":%.9f,\"targetedVariance\":%.9f,\"varianceReductionPercent\":%.3f,\"globalResetMean\":%.6f,\"targetedMean\":%.6f,\"resetMean\":%.6f,\"rawCommandGpuMs\":%.4f,\"denoisedCommandGpuMs\":%.4f,\"alphaErrors\":%u,\"bypassErrors\":%u,\"invalidMatrixRejected\":%s}\n",passed?"true":"false",(unsigned long long)after.encoded_frames,(unsigned long long)after.completed_frames,(unsigned long long)after.history_resets,variance[0],variance[1],100*(1-variance[1]/variance[0]),mean[0],mean[1],resetMean,gpuMs[0],gpuMs[1],alphaErrors,bypassErrors,rejectsInvalid?"true":"false");
