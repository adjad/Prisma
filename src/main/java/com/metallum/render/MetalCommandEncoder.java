...
final class MetalCommandEncoder implements CommandEncoderBackend {
    ...
    void encodeRt(com.metallum.rt.NativeRtScene scene,MetalGpuTexture color,MetalGpuTexture depth,float[] matrix,float[] camera,float[] sun,float[] controls,float[] transport,int width,int height,int frame) {
        submitRenderPass();endEncoder();
        scene.encodeFrame(commandBuffer().handle(),color.nativeHandle(),depth.nativeHandle(),fence.handle(),matrix,camera,sun,controls,transport,width,height,frame);
